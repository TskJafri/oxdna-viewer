...
(function (toscad) {
    ...
    function tempGlobalPos(networkMap, grid, lattice = 'honeycomb', options) {
        const latticeType = resolveLatticeKind(lattice);
        ...
        const getWeight = (a, b) => uniformWeights ? 1 : (connectionCounts.get(a)?.get(b) ?? 1);
        ...
        const enqueueDeferred = (parentId, helixId) => {
            const k = `${parentId}|${helixId}`;
            if (deferredSeen.has(k) || canonicalCoord.has(helixId))
                return;
            deferredSeen.add(k);
        ...
        const findNearestOpen = (anchorCol, anchorRow) => {
            if (!occupied.has(keyOf(anchorCol, anchorRow)))
            ...
            while (occupied.has(keyOf(fc, anchorRow)))
                fc += 1;
            return [fc, anchorRow];
        };
        ...
        const findFarPlacement = () => {
            ...
            while (occupied.has(keyOf(col, 0)))
                col += 1;
            return [col, 0];
        };
        // BFS from every root
        for (const rootId of sortedHelixIds) {
            ...
            if (trackOccupancy)
                occupied.set(keyOf(rootCol, rootRow), rootId);
            const queue = [rootId];
            ...
            while (qi < queue.length) {
                ...
                for (const c of candidates) {
                    ...
                    if (trackOccupancy)
                        occupied.set(keyOf(childCol, childRow), c.nid);
                    queue.push(c.nid);
                }
            }
        }
    ...
    function helixPositions(networkMap, grid, lattice = 'honeycomb') {
        ...
        for (const [hid, plist] of preds.entries()) {
            ...
            if (canonical)
                result.set(hid, canonical.coord);
        }
        return result;
    }
    toscad.helixPositions = helixPositions;
    ...
    // Post-hoc compaction of disconnected components in the final layout.
    //
    // During tempGlobalPos, disconnected roots are seeded at
    // (maxOccupiedCol + buffer, 0) so downstream renumberHelicesGNN sees
    // them as separate clusters and produces a clean remap. For the FINAL
    // rendered layout that gets handed to scadnano, we want them packed
    // tight against the main tree instead of trailing off to the right.
    //
    // Approach: identify connected components via the grid's crossover
    // graph. For every non-primary component (the primary is whichever
    // contains helix 0, or the smallest-id helix if 0 isn't present),
    // find the (dCol, dRow) translation of minimum Chebyshev magnitude
    // that places all of the component's cells in currently-empty slots,
    // then apply that translation. Subtree shape is preserved because
    // we translate the whole component as a rigid block.
    function compactDisconnected(helixPos, grid) {
        const connectionCounts = getConnectionCounts(grid);
        const remaining = new Set(helixPos.keys());
        if (remaining.size === 0)
            return;
        const bfsComponent = (startId) => {
            ...
            while (qi < queue.length) {
                const cur = queue[qi++];
                comp.push(cur);
                const neighbors = connectionCounts.get(cur);
                if (!neighbors)
                    continue;
        ...
        // Primary component first: whichever contains helix 0 (else the
        // smallest surviving id). Its coords stay fixed; everything else
        // packs around it.
        const primarySeed = helixPos.has(0) ? 0 : Math.min(...remaining);
        const primary = bfsComponent(primarySeed);
        const occupied = new Set();
        for (const hid of primary) {
            const p = helixPos.get(hid);
            if (p)
                occupied.add(`${p[0]},${p[1]}`);
        }
        const orderedRemaining = [...remaining].sort((a, b) => a - b);
        for (const startId of orderedRemaining) {
            if (!remaining.has(startId))
                continue;
            const comp = bfsComponent(startId);
            const cells = comp
                .map(hid => ({ hid, pos: helixPos.get(hid) }))
                .filter((c) => c.pos !== undefined);
            if (cells.length === 0)
                continue;
            // Ring-search for the tightest translation (dCol, dRow) that
            // clears every cell of this component against `occupied`.
            const fits = (dCol, dRow) => {
                for (const { pos } of cells) {
                    if (occupied.has(`${pos[0] + dCol},${pos[1] + dRow}`))
                        return false;
                }
                return true;
            };
            let bestDelta = null;
            outer: for (let d = 0; d <= 1000; d++) {
                for (let dc = -d; dc <= d; dc++) {
                    for (let dr = -d; dr <= d; dr++) {
                        if (Math.max(Math.abs(dc), Math.abs(dr)) !== d)
                            continue;
                        // Anchor the search on the component's leftmost-topmost
                        // cell so (dc, dr) means "put that cell at (dc, dr)".
                        const anchor = cells.reduce((min, c) => c.pos[0] < min[0] || (c.pos[0] === min[0] && c.pos[1] < min[1])
                            ? c.pos : min, cells[0].pos);
                        const dCol = dc - anchor[0];
                        const dRow = dr - anchor[1];
                        if (fits(dCol, dRow)) {
                            bestDelta = [dCol, dRow];
                            break outer;
                        }
            ...
            if (bestDelta) {
                const [dCol, dRow] = bestDelta;
                for (const { hid, pos } of cells) {
                    const nc = pos[0] + dCol;
                    const nr = pos[1] + dRow;
                    helixPos.set(hid, [nc, nr]);
                    occupied.add(`${nc},${nr}`);
                }
            }
        }
    }
    toscad.compactDisconnected = compactDisconnected;
    // ─────────────────────────────────────────────────────────────────────
