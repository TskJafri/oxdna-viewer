...
class ScadnanoExportManager {
    ...
    gridFingerprintLabeled(grid, helicesLength) {
    ...
    gridFingerprintCanonical(grid, helicesLength) {
        return toscad.canonicalGridFingerprint(grid, helicesLength);
    }
    ...
    helixPosFingerprintCanonical(grid, helixPos, lattice) {
        const canon = toscad.canonicalHelixPos(grid, helixPos);
    ...
    networkMapFingerprintCanonical(grid, networkMap) {
    ...
    stageFpRecord(tag, iter, fp) {
        if (!ScadnanoExportManager.DEBUG_STAGE_FINGERPRINTS && !this.stageTrace)
            return;
        const hash = ScadnanoExportManager.shortHashOf(fp);
        if (this.stageTrace)
            this.stageTrace.push({ iter, tag, hash, len: fp.length });
    ...
    stageFp(tag, iter, grid, helicesLength) {
        if (!ScadnanoExportManager.DEBUG_STAGE_FINGERPRINTS && !this.stageTrace)
            return;
    ...
    convergeLayout(helicesIn, grid, binderHelices, requestedLatticeType, wireframe) {
        ...
        for (let iter = 1; iter <= MAX_ITER; iter++) {
            ...
            if (ScadnanoExportManager.DEBUG_TIESTATS)
            ...
            this.stageFpRecord('calcGlobalPositions', iter, this.helixPosFingerprintCanonical(grid, helixPos, latticeTypeSet));
            ...
            console.log(`[scadnano] iter=${iter} canonical=${fp === prevFp ? 'SAME' : 'changed'} ` +
                `labeled=${fpLabeled === prevFpLabeled ? 'SAME' : 'changed'} ` +
                `geom=${fpGeom === prevFpGeom ? 'SAME' : 'changed'}`);
            if (ScadnanoExportManager.DEBUG_TIESTATS)
                toscad.tiestats.report(`iter=${iter}`);
    ...
    // ─────────────────────────────────────────────────────────────────────
    runLabelInvarianceTest(trialsInput, gridTypeInput, seedInput) {
        const trials = Math.max(2, Math.min(50, Math.floor(Number(trialsInput)) || 8));
        const requested = (gridTypeInput === 'square' || gridTypeInput === 'honeycomb' || gridTypeInput === 'automatic')
            ? gridTypeInput
            : 'automatic';
        ...
        try {
            for (let t = 0; t < trials; t++) {
                ...
                toscad.tiestats.reset();
                this.stageTrace = [];
                const res = this.convergeLayout(permuted.helices, grid, permuted.binderHelices, frozenLattice, false);
                const trace = this.stageTrace;
                this.stageTrace = null;
                if (t === 0)
                    frozenLattice = res.latticeType;
                const coords = toscad.normalizeCanonicalPos(toscad.canonicalHelixPos(res.grid, res.helixPos), res.latticeType);
                ...
                console.log(`[label-invariance] trial=${t} ${t === 0 ? '(identity)' : `seed=${seed}`} ` +
        ...
        console.log(`%c[label-invariance] ${trials} trials -> ${distinctLayouts} distinct layout(s), ` +
            `${distinctGrids} distinct grid(s)`, distinctLayouts === 1 && distinctGrids === 1 ? 'color:#2a2' : 'color:#c22;font-weight:bold');
        if (distinctLayouts > 1 || distinctGrids > 1) {
            const base = results[0];
            const firstBad = results.find(r => r.geomFp !== base.geomFp || r.gridFp !== base.gridFp);
            if (firstBad) {
                console.log(`[label-invariance] first divergence: trial=${firstBad.trial} seed=${firstBad.seed} ` +
                ...
                if (firstBad.helixCount !== base.helixCount) {
                    console.log(`[label-invariance] helix COUNT differs: ${base.helixCount} vs ${firstBad.helixCount} ` +
                ...
                if (firstDivergentStage >= 0) {
                    ...
                    console.log(`%c[label-invariance] label dependence ENTERS at stage #${firstDivergentStage}: ` +
                        `iter=${a ? a.iter : '?'} ${a ? a.tag : '(end of trace)'} ` +
                        `(${a ? a.hash : '-'} vs ${b ? b.hash : '-'})`, 'color:#c22;font-weight:bold');
                    ...
                    for (let i = from; i < Math.min(n, firstDivergentStage + 3); i++) {
                        ...
                        console.log(`    #${String(i).padStart(3)} iter=${base.trace[i].iter} ` +
                            `${base.trace[i].tag.padEnd(22)} ${base.trace[i].hash} vs ${firstBad.trace[i].hash}${mark}`);
                    }
                }
                else if (base.trace.length > 0) {
                    console.log(`[label-invariance] stage traces are identical — divergence appeared only in a ` +
                ...
                console.log(`[label-invariance] replay this case with: ` +
...
function registerScadnanoWindowApi() {
    ...
    window.scadnanoLabelInvarianceTest = (trials, gridType, seed) => scadnanoManager.runLabelInvarianceTest(trials, gridType, seed);
