...
namespace helix {
	 ...
	 * excluded from the angular search and assigned to their first backbone-
	 * connected duplex parent's cluster. Noise and helices without a usable
	 * axis are retained as singleton output clusters.
	 *
	 * Returns one array of helix ids per output cluster (indices into the input
	 * `helices` array). Every input helix appears in exactly one cluster.
	 */
	export function dbscan(helices: Nucleotide[][], minPts: number = 1): number[][] {
		...
		// Assign output clusters only after DBSCAN finishes. This keeps every region
