...
class ScadnanoExportManager {
    ...
    private convergeLayout(
        ...
        const DEBUG_DISABLE_RENUMBER = false;
            if (!DEBUG_DISABLE_RENUMBER) {
                ...
                console.log(`[scadnano] DEBUG_DISABLE_RENUMBER=true — skipping renumber on wireframe pass`);
            }
        ...
        let prevMergeProducts: number[][][] = [];
        // Iteration cap for the fixed-point loop.
        const MAX_ITER = 1; // DEBUG: was 7, reduced to 1 to skip the fixed-point loop
        // Renumber is gated for debugging — identity renumber would keep IDs in place.
        // Roots = helices tempGlobalPos seeds as BFS roots. Collected per
        // iteration and dumped after the loop to show which helices were
        // disconnected from helix 0's component.
        const rootSnapshots: Array<{
            iter: number;
            roots: number[];
            rootCols: Array<[number, number]>;
        }> = [];

        for (let iter = 1; iter <= MAX_ITER; iter++) {
            // Merge events recorded during this pass, folded into provenance
            // products at the end of the pass.
            const iterMerges: Array<{ keepNtIds: number[]; mergedNtIds: number[] }> = [];
            ...
            let mergedGroups: Map<number, number[][]> | undefined;
            if (prevGrid && prevMergeProducts.length > 0) {
                const productByNt = new Map<number, number[][]>();
                for (const groups of prevMergeProducts) {
                    for (const group of groups) {
                        for (const ntId of group) productByNt.set(ntId, groups);
                    }
                }
                ...
                for (let i = 0; i < helices.length; i++) {
                    ...
                    if (groups) mergedGroups.set(i, groups);
                }
            ...
            const deriveBinderHelixSet = (): Set<number> => {
                const s = new Set<number>();
                for (const [ntId, m] of grid.entries()) {
                    if (binderNtIds.has(ntId)) s.add(m.helixId);
                }
                return s;
            };

            const filterBinderEntries = (nm: Map<number, Map<number, number>>): Map<number, Map<number, number>> => {
                const bSet = deriveBinderHelixSet();
                const out = new Map<number, Map<number, number>>();
                for (const [hid, neighbors] of nm.entries()) {
                    if (bSet.has(hid)) continue;
                    const filteredNeighbors = new Map<number, number>();
                    for (const [nid, angle] of neighbors.entries()) {
                        if (bSet.has(nid)) continue;
            ...
            // ── root identification ─────────────────────────────────────────────
            // Roots = helices that tempGlobalPos will seed as a BFS root (i.e.
            // in a connected component disconnected from helix 0, or fully
            // isolated). We compute this *after* renumber/networkMap are
            // stable for this iteration. Cost: O(V+E) over networkMap.
            const findRoots = (): { roots: number[]; rootCols: Array<[number, number]> } => {
                const helixIds = new Set<number>();
                for (const [from, row] of networkMap.entries()) {
                    helixIds.add(from);
                    for (const to of row.keys()) helixIds.add(to);
                }
                ...
                while (stack.length) {
                    ...
                    for (const v of adj.get(u) ?? []) {
                        if (!reached.has(v)) { reached.add(v); stack.push(v); }
                    }
                }
                const roots: number[] = [];
                const rootCols: Array<[number, number]> = [];
                for (const hid of Array.from(helixIds).sort((a, b) => a - b)) {
                    if (reached.has(hid)) continue;
                    roots.push(hid);
                    const p = helixPos.get(hid);
                    rootCols.push(p ? [p[0], p[1]] : [NaN, NaN]);
                }
                return { roots, rootCols };
            };
            const { roots, rootCols } = findRoots();
            rootSnapshots.push({ iter, roots, rootCols });
            console.log(
                `[scadnano] iter=${iter} roots=${roots.length} ` +
                `[${roots.join(',')}] ` +
            ...
            if (!DEBUG_DISABLE_RENUMBER) {
                ...
                console.log(`[scadnano] DEBUG_DISABLE_RENUMBER=true — skipping renumber on iter ${iter}`);
            }
        ...
        // Aggregate root dump: across all iterations, what helices were
        // roots each pass? Use to confirm whether a helix is consistently
        // disconnected or only transiently unreachable.
        for (const snap of rootSnapshots) {
            const cols = snap.rootCols.map(([c, r]) => `${c},${r}`).join(' ');
            console.log(
                `[scadnano:rootdump] iter=${snap.iter} roots=[${snap.roots.join(',')}] ` +
                `rootCols=[${cols}]`
            );
        }
    ...
    // as a single group.
    private buildMergeProducts(
        mergeLog: Array<{ keepNtIds: number[]; mergedNtIds: number[] }>
    ): number[][][] {
        const products: number[][][] = [];
        const productIndexByNt = new Map<number, number>();

        const retarget = (fromIdx: number, toIdx: number) => {
            if (fromIdx === toIdx) return;
            for (const group of products[fromIdx]) {
                for (const ntId of group) productIndexByNt.set(ntId, toIdx);
            }
            products[fromIdx] = [];
        };

        for (const { keepNtIds, mergedNtIds } of mergeLog) {
            if (!keepNtIds.length || !mergedNtIds.length) continue;
            const keepIdx = productIndexByNt.get(keepNtIds[0]);
            const mergedIdx = productIndexByNt.get(mergedNtIds[0]);
            if (keepIdx !== undefined && keepIdx === mergedIdx) continue;
            ...
            if (keepIdx !== undefined) {
                ...
                for (const ntId of keepNtIds) productIndexByNt.set(ntId, target);
            }
            if (mergedIdx !== undefined) {
                retarget(mergedIdx, target);
            } else {
                for (const ntId of mergedNtIds) productIndexByNt.set(ntId, target);
            }
        }
