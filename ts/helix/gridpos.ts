...
interface LatticePhaseConfig {
    ...
    // are non-negative.
    export function helixPairCosine(
        A: number,
        B: number,
        connectionCounts: Map<number, Map<number, number>>
    ): number {
        ...
        for (const n of nbA.keys()) if (n !== A && n !== B) neighbors.add(n);
        for (const n of nbB.keys()) if (n !== A && n !== B) neighbors.add(n);
	...
	export function axisShadowOverlap(
		...
		const endpointsOf = (helix: Nucleotide[]): [THREE.Vector3, THREE.Vector3] | null => {
			const ids = new Set<number>();
			for (const nt of helix) ids.add(nt.id);
			const pointOf = (nt: Nucleotide): THREE.Vector3 => {
				const own = nt.getPos();
			...
			for (const nt of helix) {
				...
				if (d > farADist) { farADist = d; farA = nt; }
			}
			...
			for (const nt of helix) {
				...
				if (d > farBDist) { farBDist = d; farB = nt; }
			}
			if (farA === farB) return null;
    ...
    // that case.
    export function anglecomb3(
        grid: GridMap,
        helices: Nucleotide[][],
        lattice: string = 'honeycomb',
        partials: Nucleotide[][],
        hashMergePairs: helix.HashMergePair[],
        ...
        mergedPairs: Array<{
            ...
            keepNtIds: number[];
            mergedNtIds: number[];
            phase: 'hash' | 'strict' | 'queue-relaxed';
            cos: number;
            axisDot: number;
            source?: number;         // strict / queue-relaxed: bucket parent
            cell?: [number, number]; // strict / queue-relaxed: bucket cell
            witnesses?: number[];    // queue-relaxed: strong mutuals
            wasDisjoint?: boolean;   // hash: true iff gate 1 would have passed
            shadowOverlap?: boolean; // hash: true iff axis-shadow would have rejected
        }>;
    } {
        ...
        const gate23 = (A: number, B: number): { pass: boolean; axisDot: number; reason: string } => {
            const axisA = helixAxis(A);
            const axisB = helixAxis(B);
            if (!axisA || !axisB) return { pass: false, axisDot: 0, reason: 'missing-axis' };
            const axisDot = Math.abs(axisA.dot(axisB));
            if (axisDot < AXIS_DOT_THRESHOLD) return { pass: false, axisDot, reason: 'axisDot-below-threshold' };
            if (axisShadowOverlap(helices[A], helices[B])) return { pass: false, axisDot, reason: 'shadow-overlap' };
        ...
        const collectBuckets = (preds: Map<number, HelixPrediction[]>): Array<{
            parent: number; localAngle: number; coord: [number, number]; hids: Set<number>;
        }> => {
        ...
        const inAnyBucket = (
            A: number, B: number,
            buckets: Array<{ hids: Set<number> }>
        ): boolean => {
            for (const b of buckets) {
                if (b.hids.has(A) && b.hids.has(B)) return true;
            }
            return false;
        };
        ...
        const strictParentsAgree = (
            childHid: number,
            cell: [number, number],
            preds: Map<number, HelixPrediction[]>,
            cc: Map<number, Map<number, number>>
        ): boolean => {
            let sawStrong = false;
        ...
        const computeStrongMutuals = (
            ...
            for (const n of parentsA) {
                ...
                if (nToA >= 2 && nToB >= 2) out.push(n);
            }
            return out.sort((a, b) => a - b);
        };

        const minNtIdOf = (hid: number): number => {
            const nts = helices[hid] ?? [];
            let m = Number.POSITIVE_INFINITY;
            for (const nt of nts) if (nt && nt.id < m) m = nt.id;
        ...
        while (anyMergeThisIteration && iteration < MAX_ITERATIONS) {
            ...
            if (haveHashPairs && havePartials) {
                ...
                while (phase1Merged) {
                    ...
                    for (const mp of hashMergePairs) {
                        ...
                        console.log(
                            `[anglecomb3] iter=${iteration} phase=hash merged ${merged}→${keep} ` +
                        ...
                        if (!wasDisjoint) {
                            console.warn(
                                `[anglecomb3] ⚠️ non-disjoint hash merge ${merged}→${keep} — ` +
                                `merged helix will have overlapping offsets on its number line.`
                            );
                        }
                        if (shadowOverlap) {
                            console.warn(
                                `[anglecomb3] ⚠️ hash merge ${merged}→${keep} shadow-overlaps — ` +
            ...
            // ── PHASE 2: anglecomb2 strict (one merge per pass, to fixed-point) ──
            {
                ...
                while (phase2Merged) {
                    ...
                    for (const bucket of buckets) {
                        if (bucket.hids.size < 2) continue;
                        ...
                        for (let i = 0; i < sorted.length; i++) {
                            ...
                            for (let j = i + 1; j < sorted.length; j++) {
                                ...
                                if (!g23.pass) continue;
                                if (!strictParentsAgree(A, bucket.coord, preds, cc)) continue;
                                if (!strictParentsAgree(B, bucket.coord, preds, cc)) continue;
                                ...
                                console.log(
                                    `[anglecomb3] iter=${iteration} phase=strict merged ${merged}→${keep} ` +
            ...
            // ── PHASE 3: queue-drain of relaxed anglecomb2 passers ──────
            {
                ...
                const enqueue = (e: QueueEntry): void => {
                ...
                const queue = [...queueMap.values()].sort((x, y) =>
                    y.cos - x.cos || y.axisDot - x.axisDot
                );

                if (queue.length > 0) {
                    console.log(
                        `[anglecomb3] iter=${iteration} phase=queue built ${queue.length} candidate(s) ` +
                ...
                const consumedNts = new Set<number>();
                let queueMergedAny = false;
                for (const entry of queue) {
                    if (consumedNts.has(entry.minNtA) || consumedNts.has(entry.minNtB)) {
                        console.log(
                            `[anglecomb3] iter=${iteration} phase=queue-relaxed skipped (${entry.A},${entry.B}) ` +
                    ...
                    console.log(
                        `[anglecomb3] iter=${iteration} phase=queue-relaxed merged ${merged}→${keep} ` +
            ...
            console.log(
                `[anglecomb3] iter=${iteration} complete — ` +
    ...
    // ── Overlap-free placement helpers ─────────────────────────────────────
    // Every nucleotide occupies a cell identified by (helixId, direction,
    // offset). Two marks sharing all three = an overlap conflict.
    type CellIndex = Map<number, Map<string, Set<number>>>;

    function buildCellIndex(grid: GridMap): CellIndex {
        const idx: CellIndex = new Map();
        for (const [, m] of grid.entries()) {
            let hm = idx.get(m.helixId);
            if (!hm) { hm = new Map(); idx.set(m.helixId, hm); }
            const key = `${m.direction}|${m.offset}`;
            if (!hm.has(key)) hm.set(key, new Set());
    ...
    function cellFree(idx: CellIndex, hid: number, selfHid: number, dir: string, off: number): boolean {
        const hm = idx.get(hid);
        if (!hm) return true;
        const s = hm.get(`${dir}|${off}`);
        if (!s) return true;
        if (selfHid === hid) return true;
        for (const other of s) if (other !== selfHid) return false;
        return true;
    }

    // Would shifting every mark of helix hid by delta cause a collision?
    function collidesAt(grid: GridMap, idx: CellIndex, hid: number, delta: number): boolean {
        if (delta === 0) return false;
        for (const [, m] of grid.entries()) {
            if (m.helixId !== hid) continue;
            if (!cellFree(idx, hid, hid, m.direction, m.offset + delta)) return true;
        }
        return false;
    }
    ...
    function pickCollisionFreeDelta(grid: GridMap, hid: number, idealDelta: number, maxRadius = 24): number | null {
        const idx = buildCellIndex(grid);
        if (!collidesAt(grid, idx, hid, idealDelta)) return idealDelta;
        for (let radius = 1; radius <= maxRadius; radius++) {
            for (const sign of [1, -1]) {
                const cand = idealDelta + sign * radius;
                if (!collidesAt(grid, idx, hid, cand)) return cand;
            }
        }
        return null;
    }
    ...
    function applySingleShift(grid: GridMap, hid: number, delta: number) {
        if (delta === 0) return;
        for (const [, m] of grid.entries()) if (m.helixId === hid) m.offset += delta;
    ...
    export function alignMergedGroups(
        ...
        if (!mergedGroups || mergedGroups.size === 0) return;
        ...
        if (movedAny) {
            console.warn(
                `[alignMergedGroups] Merged-helix inter-group overlap resolved by min-|delta| shifts` +
                ` (sweeps=${sweepGuard - 1}${unresolvable > 0 ? `, unresolved=${unresolvable}` : ''}).`
            );
        }
        if (unresolvable > 0) {
            console.error(
                `[alignMergedGroups] ${unresolvable} merged-helix group(s) had no collision-free placement` +
