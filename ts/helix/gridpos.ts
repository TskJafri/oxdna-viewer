                ...
                for (const conflict of report.conflicts) {
                    ...
                    for (let i = 0; i < collidedHelices.length; i++) {
                        ...
                        for (let j = i + 1; j < collidedHelices.length; j++) {
                            ...
                            const scoreContested = (candidateId: number): {
                                mass: number;
                                voterCount: number;
                            } => {
                                const neighborMap = networkMap.get(candidateId);
                                if (!neighborMap) return { mass: 0, voterCount: 0 };
                                const weightRow = connectionCounts.get(candidateId);
                                let mass = 0;
                                let voterCount = 0;
                                for (const neighborId of neighborMap.keys()) {
                                    if (neighborId === sourceHelix) continue;
                                    if (neighborId === helixA || neighborId === helixB) continue;
