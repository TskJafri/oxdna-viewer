...
namespace toscad {
    ...
    export function compactDisconnected(
        helixPos: Map<number, [number, number]>,
        grid: GridMap
    ): void {
        const connectionCounts = getConnectionCounts(grid);
        const remaining = new Set<number>(helixPos.keys());
        if (remaining.size === 0) return;

        const bfsComponent = (startId: number): number[] => {
            const comp: number[] = [];
        ...
        for (const startId of orderedRemaining) {
            ...
            const cells = comp
                ...
                .filter((c): c is { hid: number; pos: [number, number] } => c.pos !== undefined);
            if (cells.length === 0) continue;

            // Ring-search for the tightest translation (dCol, dRow) that
            // clears every cell of this component against `occupied`.
            const fits = (dCol: number, dRow: number): boolean => {
                for (const { pos } of cells) {
                    if (occupied.has(`${pos[0] + dCol},${pos[1] + dRow}`)) return false;
                }
                return true;
            };

            let bestDelta: [number, number] | null = null;
            outer:
            for (let d = 0; d <= 1000; d++) {
                for (let dc = -d; dc <= d; dc++) {
                    for (let dr = -d; dr <= d; dr++) {
                        ...
                        const anchor = cells.reduce((min, c) =>
                            c.pos[0] < min[0] || (c.pos[0] === min[0] && c.pos[1] < min[1])
                                ? c.pos : min,
                            cells[0].pos);
