    ...
    export function setGrid(
        ...
        if (preserveGrid && mergedGroups && mergedGroups.size > 0) {
            ...
            if (pseudoToReal.size > 0) {
                ...
                const groupIdxByNt = new Map<number, number>();
                mergedGroups.forEach((groups) => {
                    groups.forEach((group, gi) => {
                        for (const ntId of group) groupIdxByNt.set(ntId, gi);
                    });
                });
                ...
                // direction = overlap.
                const groupOverlaps = (helixId: number, gi: number): boolean => {
                    const group = mergedGroups.get(helixId)?.[gi];
                    ...
                    for (const ntId of group) {
                        ...
                        for (const [otherNtId, other] of grid) {
                            if (other.helixId !== helixId) continue;
                            if (groupSet.has(otherNtId)) continue;
                            if (`${other.direction}|${other.offset}` === key) return true;
                        }
                    }
                    return false;
                };
                ...
                while (sweepGuard++ < 50) {
                    ...
                    mergedGroups.forEach((groups, helixId) => {
                        for (let gi = 1; gi < groups.length; gi++) {
                            if (!groupOverlaps(helixId, gi)) continue;
                            shiftGroup(helixId, gi, 1);
                            overlappedThisRound = true;
                            movedAny = true;
                        }
                    });
                    if (!overlappedThisRound) break;
                }
                if (movedAny) {
