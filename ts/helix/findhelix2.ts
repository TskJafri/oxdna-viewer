...
namespace honda {
	...
	export function generateHelix(partials: Nucleotide[][], ssdna: Nucleotide[][], ssScaffold: Nucleotide[][], deadfishies: Nucleotide[]) {
		...
		if (murdered.length && helices.length) {
			...
			const walkToHelix = (start: Nucleotide | null, dir: 'n5' | 'n3') => {
				if (!start) {
			...
			murdered.forEach(nt => {
				const via5 = walkToHelix((nt.n5 ?? null) as Nucleotide | null, 'n5');
				const via3 = walkToHelix((nt.n3 ?? null) as Nucleotide | null, 'n3');
